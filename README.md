...
> **現在の進捗：フェーズ5（敵種・ボス・ステージ進行）完了 / フェーズ6（メタ進行）着手待ち**
...
拠点でガチャを引き、装備を整え、ステージに挑み、ボスを倒して次を解放する。
収集・育成・攻略のループが完全に一周する。
**拠点強化・実績・演出の作り込みはまだ入っていない。**
...
|---|---|---|
| `update()` | 31.2 µs / フレーム | — |
| `instances.sync()` | 7.7 µs / フレーム | — |
| **JSフレームコスト合計** | **0.039 ms** | 4 ms |
...
| R | 32.0% | 31.81% | OK |
| SR | 11.0% | 10.89% | OK |
| SSR | 4.0% | **4.56%** | OK（天井ぶん上振れするのが正しい） |
