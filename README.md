...
> **現在の進捗：フェーズ2（戦闘コア）完了 / フェーズ3（レベル上げ）着手待ち**
...
3Dアリーナで敵と戦い、倒し、倒されるところまで。
**レベル上げ・ガチャ・装備・ボス・演出はまだ入っていない。**
...
### 検証結果（敵150体を密集させた最悪ケース）

| 項目 | 実測 | 予算 |
|---|---|---|
| `update()` | 13.8 µs / フレーム | — |
| `instances.sync()` | 6.8 µs / フレーム | — |
| **JSフレームコスト合計** | **0.022 ms** | 4 ms |
| **600フレームのヒープ増加** | **0.0 KB** | ループ内アロケーション0 |
| draw call | 14〜25 | 100 |
| 三角形数 | 2,566〜17,412 | 60,000 |
